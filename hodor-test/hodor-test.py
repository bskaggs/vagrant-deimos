...
class HodorScheduler(Scheduler):
    def __init__(self, sleep=10,
                       container="docker:///hodor/hodor",
                       trials=10):
...
class ExecutorScheduler(Scheduler):
    sh = "python hodor-test.py --executor"
    ...
    def __init__(self, command=sh, uris=[this], container=libmesos, trials=10):
        ...
        self.container = container
        self.messages  = []
        self.executor  = "hodor-test.%s.executor" % self.token
    def statusUpdate(self, driver, update):
        super(type(self), self).statusUpdate(driver, update)
        if self.all_tasks_done():
            sid = update.slave_id
...
log = logging.getLogger("hodor-test")
